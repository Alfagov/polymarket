...
namespace polymarket::gamma {
    class GammaClient {
        ...
        std::vector<Event> fetch_events(const Parameters &params, const std::string &cursor = "");
        std::vector<Event> fetch_all_events(const Parameters &params);
        Event fetch_event_by_id(int id);
        Event fetch_event_by_slug(const std::string &slug);
        std::vector<Tag> fetch_events_tags(int id);

        std::vector<Market> fetch_markets(const Parameters &params, const std::string &cursor = "");
        std::vector<Market> fetch_all_markets(const Parameters &params);
    ...
    private:
        ...
        std::string build_query_string(const Parameters& p);
